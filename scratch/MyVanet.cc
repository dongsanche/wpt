...
struct PacketLog
{
  ...
  bool
  operator< (const PacketLog &plog) const
  {
...
ChooseAnchor ()
{
  ...
  for (int i = 0; i < nNodes; i++)
    {
      double x, y;
      x = NodeList ().GetNode (i)->GetObject<MobilityModel> ()->GetPosition ().x;
      y = NodeList ().GetNode (i)->GetObject<MobilityModel> ()->GetPosition ().y;
      anchor temp (x, y);
  ...
  for (int i = 0; i < nodes.size (); i++)
    {
      for (int j = 0; j < nodes.size (); j++)
        {
          double temp = (sqrt ((nodes[i].x - nodes[j].x) * (nodes[i].x - nodes[j].x)) +
                         sqrt ((nodes[i].y - nodes[j].y) * (nodes[i].y - nodes[j].y))) /
                        max;
          m[i][j] = temp;
          m[j][i] = temp;
        }
...
SendRandomPacket (int n)
{
  ...
  Address SinkAddress = InetSocketAddress (
      NodeList ().GetNode (sinkID)->GetObject<Ipv4> ()->GetAddress (1, 0).GetLocal (), sinkPort);
      ...
      if (dis >= DistanceRange - 200 && dis <= DistanceRange + 200)
        {
          Ptr<Socket> SendSocket =
              Socket::CreateSocket (NodeList ().GetNode (srcID), UdpSocketFactory::GetTypeId ());
...
void
SendSpecificPacket (int srcID, int sinkID)
{
  int m_packetSize = 1024;
  int sinkPort = 8080;
  SendCount++;
...
map<int,int> h1;
  ...
  for (int i = 0; i < nNodes; i++)
    {
      Vector temp (rand () % 100, rand () % 100, 0);
      Ptr<ConstantPositionMobilityModel> tempPosition =
...
for(int i=0;i<100;i++)
{
  ...
  wifi.SetRemoteStationManager ("ns3::ConstantRateWifiManager", "DataMode",
                                ...
                                UintegerValue (0));
  auto VDevices = wifi.Install (wifiPhy, wifiMac, Nodes);
  ...
  Simulator::Schedule (Seconds (20), &ChooseAnchor);
  ...
  NS_LOG_UNCOND ("Sent:" << SendCount << " Received:" << recount << " Drop:" << DropCount
                         << " delay:" << (double) allTime / recount / 1000000 << "ms");
